function TaskModalDelete(props) {
  const handleClickParentDiv = (event) => {
    if (event.target === event.currentTarget) {
      props.onDeleteTaskHandler(props.index, false); //cierro el modal
    }
  };
  ...
  const deleteTask = async () => {
    const response = await fetch("http://localhost:3100/list/delete-task", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ listId: props.listId, taskId: props.taskId }),
    });

    const responseData = await response.json();

    props.onDeleteTaskHandler(props.index, false, true);

    props.onDecreaseTasks();
      ...
      <div className="flex flex-col w-[40vw]  bg-white rounded-[4px] py-[1.5rem] px-[2.5rem]">
        ...
        <div className="flex justify-end gap-[1.5rem] font-medium">
          ...
          <button
            ...
            onClick={() => props.onDeleteTaskHandler(props.index, false)}
          >
            No
          </button>
        </div>
